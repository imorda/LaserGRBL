	...
	public class GrblFile : IEnumerable<GrblCommand>
    {
		...
		private class XSegment : ColorSegment
		{
			...
			public override string ToGCodeNumber(ref int cumX, ref int cumY, L2LConf c)
			{
				...
				if (c.pwm)
                {
                    //return string.Format("X{0} {1} S{2}", formatnumber(cumX, c.oX, c), c.lOn, mColor);
                }
		...
		private class YSegment : ColorSegment
		{
			...
			public override string ToGCodeNumber(ref int cumX, ref int cumY, L2LConf c)
			{
				...
				if (c.pwm)
                {
                    //return string.Format("Y{0} {1} S{2}", formatnumber(cumY, c.oY, c), c.lOn, mColor);
                }
		...
		private class DSegment : ColorSegment
		{
			...
			public override string ToGCodeNumber(ref int cumX, ref int cumY, GrblFile.L2LConf c)
			{
				...
				if (c.pwm)
                {
                    //return string.Format("X{0} Y{1} {2} S{3}", formatnumber(cumX, c.oX, c), formatnumber(cumY, c.oY, c), c.lOn, mColor);
                }
		...
		private void ImageLine2Line(Bitmap bmp, L2LConf c)
		{
			...
			foreach (ColorSegment seg in segments)
			{
				...
				fast = seg.Fast(c);
                int mColor = seg.getColor();

				if (changeGMode)
                {
                    temp.Add(new GrblCommand(String.Format("{0} S{1}", c.lOn, mColor)));
			...
			foreach (GrblCommand cmd in temp)
			{
				try
				{
					...
					if (oldcumulate && !cumulate) //cumulate down front -> flush
					{
						if (c.pwm)
                        {
                            //rv.Add(new GrblCommand(string.Format("G0 X{0} Y{1} {2} S0", formatnumber((double)curX), formatnumber((double)curY), c.lOff)));
                            rv.Add(new GrblCommand(string.Format("{0}\nG0 X{1} Y{2}", c.lOff, formatnumber((double)curX), formatnumber((double)curY))));
                        }
