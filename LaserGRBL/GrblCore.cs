...
namespace LaserGRBL
{
	...
	public class GrblCore
	{
			...
			if (HasProgram)
			{
				...
				using (System.Windows.Forms.SaveFileDialog ofd = new System.Windows.Forms.SaveFileDialog())
				{
                    ...
                    if (lastFN != null)
                    {
                        ...
                        ofd.FileName = System.IO.Path.Combine(path, fn + ".gcode");
                    }

                    ofd.Filter = "GCODE Files|*.gcode";
            ...
            if (CanAbortProgram)
            {
                try
                {
                    ...
                    lock (this)
                    {
                        ...
                        mQueue.Enqueue(new GrblCommand("M107 P1")); //shut down laser
                    }
                }
                catch (Exception ex)
                {
		...
		private void RunProgramFromStart(bool homing)
		{
			lock (this)
			{
                ...
                if (homing)
                {
                    mQueuePtr.Enqueue(new GrblCommand("G90"));
                    mQueuePtr.Enqueue(new GrblCommand("M107 P1"));
                    mQueuePtr.Enqueue(new GrblCommand("G28 X Y"));
                    mQueuePtr.Enqueue(new GrblCommand("G1 F3000 X104 Y127.3"));
                    mQueuePtr.Enqueue(new GrblCommand("G28 Z"));
                    mQueuePtr.Enqueue(new GrblCommand("G1 F200 Z" + offset.ToString()));
                    mQueuePtr.Enqueue(new GrblCommand("G1 F3000 X79 Y25"));
                    mQueuePtr.Enqueue(new GrblCommand("G92 X0 Y0 Z0"));
                    mQueuePtr.Enqueue(new GrblCommand("" + "M106 P1 S1"));
                    mQueuePtr.Enqueue(new GrblCommand("G21"));
		...
		private void ContinueProgramFromKnown(int position, bool homing, bool setwco)
		{
			lock (this)
			{
                ...
                if (homing)
                {
                    ...
                    EnqueueCommand(new GrblCommand("M107 P1"));
                    EnqueueCommand(new GrblCommand("G28 X Y"));
                    EnqueueCommand(new GrblCommand("G1 F3000 X104 Y127.3"));
                    EnqueueCommand(new GrblCommand("G28 Z"));
                    EnqueueCommand(new GrblCommand("G1 F200 Z" + offset.ToString()));
                    EnqueueCommand(new GrblCommand("G1 F3000 X79 Y25"));
                    EnqueueCommand(new GrblCommand("G92 X0 Y0 Z0"));
                    EnqueueCommand(new GrblCommand("" + "M106 P1 S1"));
                    EnqueueCommand(new GrblCommand("G21"));
				...
				mQueuePtr.Enqueue(new GrblCommand(string.Format("M107 P1"))); //fast go to the computed position with laser off and set speed and power
				mQueuePtr.Enqueue(new GrblCommand(string.Format("G0 {0} {1} {2} {3}", spb.X, spb.Y, spb.Z, spb.F))); //fast go to the computed position with laser off and set speed and power
		...
		{ if (CanResumeHold)
            {
                ...
                {
                    SendImmediate("M108\n");
                    var items = mPending.ToArray();
                    mPending.Clear();
                    mPending.Enqueue(new GrblCommand("M108"));
                    foreach (var item in items)
                        mPending.Enqueue(item);
                }
                }
        }

		public void FeedHold()
		{ if (CanFeedHold) SendImmediate("M0\n"); }
		...
		private void QueryPosition()
        {
            ...
            {
                gotPrev = false;
                var items = mQueue.ToArray();
                mQueue.Clear();
                mQueue.Enqueue(new GrblCommand("M114"));
                foreach (var item in items)
                    mQueue.Enqueue(item);
            }
            }
        ...
        private void SendImmediate(string inp)
        {
            lock(this)
            {
                if (com.IsOpen) com.Write(inp);
        ...
        private void InternalReset(bool grbl)
		{
			lock (this)
			{
				...
				mTarOvLinear = mTarOvRapids = mTarOvPower = 100;
                    ...
                    mQueue.Clear(); //flush the queue of item to send
                    if (grbl) mQueue.Enqueue(new GrblCommand("M112")); //shut down laser
            }
		...
		internal void JogHome()
		{
			if (JogEnabled)
			{
					...
					EnqueueCommand(new GrblCommand(string.Format("G0 X0 Y0 F{0}", JogSpeed)));
			}
		}
		...
		private void StartTX()
		{
			lock (this)
			{
				InternalReset((bool)Settings.GetObject("Reset Grbl On Connect", false));
				QueryPosition();
				QueryTimer.Start();
			}
		}
				...
				if ((rline = WaitComLineOrDisconnect()) != null)
				{
					if (rline.Length > 0)
					{
						lock (this)
						{
                            ...
                            else if (rline.StartsWith("X:") || rline.ToLower().Contains("busy"))
		...
		private void ManageRealTimeStatus(string rline)
		{
			try
			{
				...
				GrblVersionInfo rversion = StatusReportVersion(rline);
					...
					ParseMachineStatus(rline);

						if (rline.StartsWith("X:"))
							ParseWPos(rline);
		...
		{
            string wpos = p.Substring(2, p.Length - 2).Replace("Y:", "").Replace("Z:", "");
		...
		{
            MacStatus var = MacStatus.Idle;
            if (data.ToLower().Contains("busy"))
            {
                if (data.ToLower().Contains("paused"))
                    var = MacStatus.Hold;
                else
                    var = MacStatus.Run;
            }
		...
		private string EvaluateCB(System.Text.RegularExpressions.Match m)
		{
			try
			{
                ...
                exp.AddSetVariable("offset", offset);
	...
	public class GrblConf : System.Collections.Generic.IEnumerable<System.Collections.Generic.KeyValuePair<int, decimal>>
	{
        ...
        { get { return true; } }
                //return ReadWithDefault(Version9 ? 22 : 17, 1) != 0; } }
